...
void memory_init() {
	...
	asm ("movq %0, %%rax; movq %%rax, %%cr3" : : "m" ((uint64_t) kernel_p4) : "rax");
...
void virtual_alloc_to(struct page_table *p4, uint64_t vaddr, uint64_t page_count) {
	for (uint64_t page = 0; page < page_count; page++)
		virtual_map(p4, vaddr + PAGE_SIZE * page, frame_alloc(1) * PAGE_SIZE, 1);
}

void* virtual_alloc(struct page_table *p4, uint64_t page_count) {
...
void heap_init(uint64_t size) {
	heap.size = size;
	heap.start = (uint64_t) virtual_alloc(kernel_p4, heap.size / PAGE_SIZE);
